...
func (p linux) checkForLvm(realPath string) string {
	...
	if strings.Contains(stdout, ` TYPE="LVM2_member"`) {
		...
		if err != nil {
			p.logger.Info(logTag, "Lvm device: %s not found, output: %s, stderr: %s", lvmDevice, stdout, stderr)

			// not really sure which of the below commands fixes it, running all of them
			stdout, stderr, _, _ := p.cmdRunner.RunCommand("sh", "-c", "vgchange -a y vgStoreData")
			p.logger.Info(logTag, "vgchange -a y vgStoreData, stdout: %s, stderr: %s", stdout, stderr)

			stdout, stderr, _, _ = p.cmdRunner.RunCommand("sh", "-c", "vgscan --mknodes -v")
			p.logger.Info(logTag, "vgscan --mknodes -v, output: %s, stderr: %s", stdout, stderr)

			stdout, stderr, _, _ = p.cmdRunner.RunCommand("sh", "-c", "pvscan")
			p.logger.Info(logTag, "pvscan, output: %s, stderr: %s", stdout, stderr)

			stdout, stderr, _, _ = p.cmdRunner.RunCommand("sh", "-c", "vgscan")
			p.logger.Info(logTag, "vgscan, output: %s, stderr: %s", stdout, stderr)

			stdout, stderr, _, _ = p.cmdRunner.RunCommand("sh", "-c", "lvscan")
